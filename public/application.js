...
var onCreateGame = function onCreateGame(event) {
  ...
  gameApi.createGame(data).then(gameUi.onCreateGameSuccess).catch(gameUi.onCreateGameFailure);
};
...
var onUpdateGame = function onUpdateGame(data) {
  ...
  // }
  // if (game.id.length !== 0) {
  gameApi.update(data).then(gameUi.onUpdateGameSuccess).catch(gameUi.onUpdateGameFailure);
  // } else {
...
var game = {
  ...
  checkForWinMsg: function checkForWinMsg(affection) {
    $('#message').html(affection + ' wins!');
  }
};

var mark = function mark(index) {
  // console.log(index)
  ...
  if (game.checkForWin() === false) {
    ...
    if (game.marked(index) === false) {
      ...
      var data = { affection: affection, gameOver: gameOver, index: index };
